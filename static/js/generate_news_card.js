function generate_news_card(info, card_id) {
    ...
    let news_element_str = `<div class="container-fluid">
                                    ...
                                    <div class="form-floating" hidden="hidden" id="news_title_input_container${card_id}">
                                    ...
                                    <div class="form-floating" hidden="hidden" id="news_text_input_container${card_id}">
    ...
    let privacy;
    if (graph_info.privacy) {
        privacy = 'Приватный';
    } else {
        privacy = 'Публичный';
    }
    let graph_element_str = `<img class="card-img-top rounded-start-0" src="data:image/png;base64,${graph_info.preview}" alt="graph image">
                                    <div class="card-body">
                                      <h5 class="card-title">${graph_info.name}</h5>
                                      <p class="card-text">${privacy}</p>
                                      <div class="d-flex flex-row justify-content-end">
                                        <a href="/graph/${graph_info.id}" class="btn btn-primary">Открыть</a>
                                      </div>
                                    </div>`
    ...
    return `<div class="d-flex flex-row p-0">
                    <div class="card rounded-end-0">
                        ...
                        <div class="card-footer d-flex flex-row justify-content-end p-1">
                            <div class="btn p-1 text-danger" onclick="delete_news(${news_info.id}, ${card_id})" id="del_btn${card_id}"> Удалить</div>
                            <div class="btn p-1 text-secondary" onclick="edit_news_mode(${card_id})" id="edit_btn${card_id}" > Редактировать</div>
                            <div class="btn p-1 text-secondary" hidden="hidden" id="cancel_btn${card_id}" onclick="normal_news_mode(${card_id})"> Отменить</div>
                            <div class="btn p-1 text-primary" hidden="hidden" id="save_btn${card_id}"> Сохранить</div>
                        </div>  
                    </div>
